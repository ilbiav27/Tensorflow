...
class Vgg16:
    def __init__(self,fig, vgg16_npy_path=None):
        self.fig = fig
    ...
    def build(self, rgb):
        ...
        ''' For printing RGB each other
        shape = bgr.get_shape().as_list()
        img1 = tf.transpose(bgr,[3,0,1,2])
        #img1,img2 = tf.split(axis=3, num_or_size_splits=[1,relu.get_shape().as_list()[3]-1], value=relu)
        
        #show filter
        for i in range(shape[3]):
            img2 = tf.reshape(img1[i],[3,shape[1],shape[2],1])
            tf.summary.image("RGB_print",img2,3)
        '''
        ...
        #self.train(self.total_loss, global_step)
    ...
    def conv_layer(self, bottom, name):
        with tf.variable_scope(name):
            ...
            img1 = tf.transpose(relu,[3,0,1,2])
            #img1,img2 = tf.split(axis=3, num_or_size_splits=[1,relu.get_shape().as_list()[3]-1], value=relu)
            
            #show filter
            for i in range(shape[3]):
                img2 = tf.reshape(img1[i],[3,shape[1],shape[2],1])
                tf.summary.image(name,img2,3)
